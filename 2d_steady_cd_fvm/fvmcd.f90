!------------------------------------------------------------------------------
	...
	implicit none
	save
	integer::scheme_choice,n,top_bc,bottom_bc,right_bc,left_bc,heat_flux
	double precision::u_vel,v_vel,fv,dv,fh,dh,u_avg
	type scaler_node_data
		integer::bc
		double precision::x,y,temp,residual
	...
	double precision,dimension(100)::temp_wall_top,temp_wall_bottom,temp_avg
...
!------------------------------------------------------------------------------
	...
	do ii=1,n
		do jj=1,n
			if(scaler_node(ii,jj)%residual>0.001) then
				conv_stat = conv_stat + 1
			end if
		end do
	end do
	if(conv_stat == 0) then
		print*, "Solution coverged at iteration number",i
		!call compute_avg
		call write_data
		STOP
	end if
...
!- 2D coefficient settings ---------------------------------------------------
	...
	do xx = 2,(n-1)
		do yy = 2,(n-1)
			...
			scaler_node(xx,yy)%ap = scaler_node(xx,yy)%ae + scaler_node(xx,yy)%aw + scaler_node(xx,yy)%an &
						&+ scaler_node(xx,yy)%as - scaler_node(xx,yy)%sp 
		end do
	end do
	...
	do xx = 2,(n-1)
		...
		scaler_node(xx,n)%ap = scaler_node(xx,n)%ae + scaler_node(xx,n)%aw + scaler_node(xx,n)%an + &
					& scaler_node(xx,n)%as - scaler_node(xx,n)%sp 
		...
		scaler_node(xx,1)%ap = scaler_node(xx,1)%ae + scaler_node(xx,1)%aw + scaler_node(xx,1)%an +&
					& scaler_node(xx,1)%as - scaler_node(xx,1)%sp 
	...
	do yy = 2,(n-1)
		...
		scaler_node(1,yy)%ap = scaler_node(1,yy)%ae + scaler_node(1,yy)%aw + scaler_node(1,yy)%an +&
					& scaler_node(1,yy)%as - scaler_node(1,yy)%sp
		...
		scaler_node(n,yy)%ap = scaler_node(n,yy)%ae + scaler_node(n,yy)%aw + scaler_node(n,yy)%an +&
					& scaler_node(n,yy)%as - scaler_node(n,yy)%sp
	...
	!-------------CV 4 -----top left corner--------------------------------------
		...
		scaler_node(1,n)%ap = scaler_node(1,n)%ae + scaler_node(1,n)%aw + scaler_node(1,n)%an + &
					&scaler_node(1,n)%as - scaler_node(1,n)%sp
	...
	!-------------CV 3 -----top right corner--------------------------------------
		...
		scaler_node(n,n)%ap = scaler_node(n,n)%ae + scaler_node(n,n)%aw + scaler_node(n,n)%an + &
					&scaler_node(n,n)%as - scaler_node(n,n)%sp
	...
	!---------------CV 1 ------bottom left corner----------------------------------------
		...
		scaler_node(1,1)%ap = scaler_node(1,1)%ae + scaler_node(1,1)%aw + scaler_node(1,1)%an +&
					& scaler_node(1,1)%as - scaler_node(1,1)%sp
	...
	!-----------------CV 2------------ bottom right corner------------------------------------
		...
		scaler_node(n,1)%ap = scaler_node(n,1)%ae + scaler_node(n,1)%aw + scaler_node(n,1)%an +&
					& scaler_node(n,1)%as - scaler_node(n,1)%sp
...
!-------------------------------------------------------------------------------------------------------------
	...
	do i=1,n
		if(col==1) then
		 source(i) = scaler_node(col,i)%su + scaler_node(col,i)%ae*scaler_node((col+1),i)%temp	
		else if(col==n) then
		 source(i) = scaler_node(col,i)%su + scaler_node(col,i)%aw*scaler_node((col-1),i)%temp
		else if((col/=1).and.(col/=n)) then
		 source(i) = scaler_node(col,i)%su + scaler_node(col,i)%aw*scaler_node((col-1),i)%temp &
				&+ scaler_node(col,i)%ae*scaler_node((col+1),i)%temp
		end if
	end do
...
!-------------------------------------------------------------------------------------------------------
	...
	integer::i,j,done
	double precision::temp_avg_temp=0,difference_1,difference_2,difference_3
	...
	PRINT*, u_avg
	do i=1,n
	 temp_wall_top(i) = scaler_node(i,n)%temp + (heat_flux*0.0001)/(2*0.6)
	 temp_wall_bottom(i) = scaler_node(i,1)%temp + (heat_flux*0.0001)/(2*0.6)
	end do
	...
	do i=1,n
	 temp_avg_temp = 0.0d0
	 do j=1,n
		temp_avg_temp = temp_avg_temp + scaler_node(i,j)%temp*u(j)
	 end do
	 temp_avg(i)=temp_avg_temp/u_avg
	end do


	
		difference_1 = temp_wall_top(80) - temp_wall_top(79)
		difference_2 = temp_avg(80) - temp_avg(79)
		difference_3 = temp_wall_bottom(80) - temp_wall_bottom(79)
